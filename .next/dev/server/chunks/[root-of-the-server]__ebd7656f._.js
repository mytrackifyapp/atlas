 ...
 */ function alternatePublicOrigins() {
    ...
    try {
        ...
        const alt = u.hostname.startsWith("www.") ? `${u.protocol}//${u.hostname.slice(4)}` : `${u.protocol}//www.${u.hostname}`;
        if (alt !== u.origin) {
            extra.add(alt);
        }
    } catch  {
...
"[project]/app/api/founder/stats/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        const activeFundraise = await db.collection("fundraises").findOne({
            userId,
            status: "active"
        });
        // Get all fundraises for this founder
        const allFundraises = await db.collection("fundraises").find({
            userId
        }).sort({
            createdAt: -1
        }).toArray();
        // Get investor interests
        const investorInterests = await db.collection("investor_interests").find({
            userId
        }).sort({
            createdAt: -1
        }).limit(5).toArray();
        // Get company metrics (revenue, expenses, cash flow)
        const companyMetrics = await db.collection("company_metrics").find({
            userId
        }).sort({
            month: -1
        }).limit(6).toArray();
        ...
        const revenueGrowth = previousMetric && previousMetric.revenue > 0 ? ((monthlyRevenue - previousMetric.revenue) / previousMetric.revenue * 100).toFixed(0) : "0";
        // Calculate burn rate change
        const burnRateChange = previousMetric && previousMetric.expenses > 0 ? ((burnRate - previousMetric.expenses) / previousMetric.expenses * 100).toFixed(0) : "0";
        // Get active investors count
        const activeInvestors = await db.collection("investor_interests").countDocuments({
            userId,
            status: {
                $in: [
                    "interested",
        ...
        thisMonth.setDate(1);
        const newInvestorsThisMonth = await db.collection("investor_interests").countDocuments({
            userId,
            createdAt: {
                $gte: thisMonth
            }
        ...
        const revenueData = companyMetrics.slice().reverse().map((metric)=>({
        ...
        for(let i = 5; i >= 0; i--){
            ...
            filledRevenueData.push(existing || {
                month: monthName,
                revenue: 0,
                expenses: 0
            });
        ...
        const cashFlowData = companyMetrics.slice().reverse().map((metric)=>({
                month: metric.month || "Unknown",
                cash: metric.cash || 0
            }));
        ...
        for(let i = 5; i >= 0; i--){
            ...
            filledCashFlowData.push(existing || {
                month: monthName,
                cash: 0
            });
        }
        ...
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$10_react$2d$dom$40$19$2e$2$2e$0_react$40$19$2e$2$2e$0_$5f$react$40$19$2e$2$2e$0$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            ...
            data: {
                ...
                currentFundraise: activeFundraise ? {
                    ...
                    percentage: activeFundraise.targetAmount ? Math.round((activeFundraise.committedAmount || 0) / activeFundraise.targetAmount * 100) : 0,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__ebd7656f._.js.map